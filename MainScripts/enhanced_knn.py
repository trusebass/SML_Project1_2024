#!/usr/bin/env python3
"""
Enhanced KNeighborsRegressor Pipeline for Distance Estimation
This script implements advanced image preprocessing techniques 
optimized for KNeighborsRegressor to improve distance estimation.
Building on the successful RGB KNN implementation that achieved MAE of 13.0.
"""
...
class EnhancedImagePreprocessor(BaseEstimator, TransformerMixin):
    """
    Enhanced image preprocessor with expanded preprocessing techniques
    """
    def __init__(self, use_edges=True, use_histogram_eq=True, use_clahe=True,
                 use_color_spaces=True, use_lbp=False, use_gabor=False,
                 use_advanced_edges=False, use_morphology=False, use_hog=False, 
                 use_superpixels=False, normalize_features=True):
        ...
        for i, flat_img in enumerate(X):
            ...
            try:
                ...
                if self.is_rgb_input:
                    ...
                    self._add_basic_features(gray_img, feature_list)
                    
                    # Color space transformations
                    if self.use_color_spaces:
                        self._add_color_space_features(img, feature_list)
                    
                    # LBP (Local Binary Pattern) features
                    if self.use_lbp:
                        self._add_lbp_features(gray_img, feature_list)
                    
                    # Gabor filter features
                    if self.use_gabor:
                        self._add_gabor_features(gray_img, feature_list)
                        
                    # Advanced edge detection
                    if self.use_advanced_edges:
                        self._add_advanced_edge_features(gray_img, feature_list)
                        
                    # Morphological operations
                    if self.use_morphology:
                        self._add_morphology_features(gray_img, feature_list)
                    
                    # HOG features
                    if self.use_hog:
                        self._add_hog_features(gray_img, feature_list, img_width)
                        
                    # Superpixel segmentation
                    if self.use_superpixels:
                        self._add_superpixel_features(img, gray_img, feature_list)
    ...
    def _add_color_space_features(self, rgb_img, feature_list):
        """Add features from different color spaces"""
        # HSV color space (good for handling lighting variations)
        hsv_img = rgb2hsv(rgb_img)
        feature_list.append(hsv_img[:,:,0].flatten())  # Hue channel
        feature_list.append(hsv_img[:,:,1].flatten())  # Saturation channel
        feature_list.append(hsv_img[:,:,2].flatten())  # Value channel
        
        # YUV color space approximation
        y = 0.299 * rgb_img[:,:,0] + 0.587 * rgb_img[:,:,1] + 0.114 * rgb_img[:,:,2]
        u = 0.492 * (rgb_img[:,:,2] - y)
        v = 0.877 * (rgb_img[:,:,0] - y)
        feature_list.append(u.flatten())  # U channel
        feature_list.append(v.flatten())  # V channel
    ...
    def _add_superpixel_features(self, rgb_img, gray_img, feature_list):
        """Add features derived from superpixel segmentation"""
        try:
            # SLIC superpixel segmentation
            n_segments = max(10, int(rgb_img.shape[0] * rgb_img.shape[1] / 100))
            segments = slic(rgb_img, n_segments=n_segments, compactness=10, sigma=1)
            
            # Extract mean color/intensity for each segment
            for segment_id in np.unique(segments):
                mask = segments == segment_id
                # Mean color in RGB image
                if self.is_rgb_input:
                    for channel in range(rgb_img.shape[2]):
                        feature_list.append(np.array([np.mean(rgb_img[:,:,channel][mask])]))
                # Mean intensity in grayscale image
                feature_list.append(np.array([np.mean(gray_img[mask])]))
        except:
            pass
...
def run_enhanced_knn_grid_search(X_train, y_train):
    ...
    param_grid = {
        ...
        'preprocessor__use_color_spaces': [True, False],
        ...
        'preprocessor__use_superpixels': [False, True],  # Slower, set to True for more features
...
if __name__ == "__main__":
    print("Starting Enhanced KNN Pipeline...")
    start_time = time.time()
    ...
    if not FINAL_SUBMISSION:
        ...
        use_grid_search = True  # Set to False for faster focused training
        ...
        try:
            ...
            # model_name = "enhanced_knn_full_training_model.pkl"
